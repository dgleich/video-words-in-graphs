...
img = get_image("Hello world!")

## Awesome! Now we have the image we want to use to figure out where to put things. 
# Images in Julia are just arrays. 
typeof(img)

## Here, the alpha channel has all the interesting information
using CairoMakie
heatmap(alpha.(img)) # this plots the alpha channel of the image as a heatmap
## Images and Arrays in Julia are "transposed" and flipped...
heatmap(alpha.(img)')
## Images and Arrays in Julia are "transposed" and flipped...
heatmap(alpha.(img)', axis=(yreversed=true,)) # I'm still learning Makie... gonna check cheat sheet! 

## Save the sampling matrix in R
R = Float64.(alpha.(img) )
...
x,y = sample_from_matrix(R, 10000)
scatter(y, -x) # do flip! 
## This gives us coordinates sampled from the image itself! That's almost step 1. 
# And we are back!
# Let's add some extra points that aren't from the image itself but scattered around.
x,y = sample_from_matrix(R .+ 0.1, 10000)
scatter(y, -x, axis=(aspect=DataAspect(),), markersize=2) # do flip! ... checking cheat sheet how to make the aspect ratio based on data
...
x,y = sample_from_matrix(pad_and_center(R;ypad=1.0) .+ 0.1, 10000)
scatter(y, -x, axis=(aspect=DataAspect(),), markersize=2)
## There, our points look nicely distributed now... but can I figure out what was the text?
...
## Switch to GLMakie to look more closely!
using GLMakie
GLMakie.activate!()


# That has too many edges! 
